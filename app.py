...
def connect(sid, environ):
    sio.emit('problems', [p['name'] for p in problems])

@sio.on('heartbeat')
def run(sid):
    return sio.emit("heartbeat")
...
def run(sid, data):
    ...
    if not code or not lang:
        return sio.emit('err', '[ERROR] No code or language submitted!')

    if lang not in sandbox.langs.keys():
        return sio.emit('err', '[ERROR] Invalid language.')

    if problem:
        problem = [p for p in problems if p['name'] == problem][0]
        if not problem:
            return sio.emit('err', '[ERROR] Invalid task.')
...
def outputHandler(result):
    if result['stderr']:
        ...
        sio.emit('err', f"{Fore.RED}{result['stderr'].decode()}")

    if result['type'] == 'compile':
        return

    sio.emit('out', f"{Fore.WHITE}{result['stdout'].decode()}")
